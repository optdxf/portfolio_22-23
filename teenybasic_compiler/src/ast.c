...
node_t *copy_ast(node_t *node) {
    if (node == NULL) return NULL;
    switch (node->type) {
        case NUM: return init_num_node(((num_node_t*)node)->value);
        case VAR: return init_var_node(((var_node_t*)node)->name);
        case BINARY_OP: {
            binary_node_t *b = (binary_node_t*)node;
            return init_binary_node(b->op, copy_ast(b->left), copy_ast(b->right));
        }
        case SEQUENCE: {
            sequence_node_t *s = (sequence_node_t*)node;
            node_t **statements = malloc(sizeof(*statements) * s->statement_count);
            for (size_t i = 0; i < s->statement_count; ++i) {
                statements[i] = copy_ast(s->statements[i]);
            }
            return init_sequence_node(s->statement_count, statements);
        }
        case PRINT: return init_print_node(copy_ast(((print_node_t*)node)->expr));
        case LET: {
            let_node_t *l = (let_node_t*)node;
            return init_let_node(l->var, copy_ast(l->value));
        }
        case IF: {
            if_node_t *i = (if_node_t*)node;
            return init_if_node((binary_node_t*)copy_ast((node_t*)i->condition), copy_ast(i->if_branch), copy_ast(i->else_branch));
        }
        case WHILE: {
            while_node_t *w = (while_node_t*)node;
            return init_while_node((binary_node_t*)copy_ast((node_t*)w->condition), copy_ast(w->body));
        }
    }
}